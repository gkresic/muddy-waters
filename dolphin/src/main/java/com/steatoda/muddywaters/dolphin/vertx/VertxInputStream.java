...
// stolen from Quarkus project: https://github.com/quarkusio/quarkus/blob/main/extensions/vertx-http/runtime/src/main/java/io/quarkus/vertx/http/runtime/VertxInputStream.java
public class VertxInputStream extends InputStream {

	public static final String MAX_REQUEST_SIZE_KEY = "com.steatoda.muddywaters.dolphin.max-request-size";
    ...
    public void close() throws IOException {
        if (closed) {
			Log.warn("Vert.x input stream closed AGAIN");
			return;
        }
        Log.info("Vert.x input stream closed");
        closed = true;
        try {
            while (!finished) {
                readIntoBuffer();
        ...
        public VertxBlockingInput(HttpServerRequest request, long timeout) {
            ...
            synchronized (connection) {
                if (!connection.channel().isOpen()) {
                	...
                	Log.error("Creating VertxBlockingInput on CLOSED request?!");
                    eof = true;
                }
            }
        }
                ...
                while (input1 == null && !eof && readException == null) {
                    ...
                    try {
                        if (Context.isOnEventLoopThread()) {
                            throw new RuntimeException("Attempting a blocking read on io thread");
                        }
                        waiting = true;
